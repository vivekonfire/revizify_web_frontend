...
const MoreOptions = ({ user, course_id }) => {
  ...
  const onClick = () => {
    if (router.pathname === "/create") dispatch(archiveCourse(course_id));
      ...
      dispatch(restoreArchiveCourses(course_id));
    else dispatch(deleteDownloadCourses(course_id));
  };

  return (
    <div
      className="position-absolute px-4 py-2 bg-black rounded-sm bottom-0 end-0 mx-5 my-4"
      onClick={(e) => {
        e.stopPropagation();
      }}
    >
      <div className="text-white mb-2">
        ...
        >
          <BiArchiveIn />
          <p className="p-0 m-0">
            {pathname == "/archive" ? "Restore" : "Archive"}
          </p>
        </div>
      </div>
      {pathname !== "/archive" ? (
